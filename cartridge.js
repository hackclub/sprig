...
class Cartridge {
  ...
  async generateUploadURL() {
    const result = await fetch(`https://vt4x133ukg.execute-api.eu-west-1.amazonaws.com/default/getPresignedURL?id=${this.id()}`).then(r => r.json())
  ...
  async upload() {
    try {
      ...
      await fetch(uploadURL, {
        ...
        method: "PUT",
        body: JSON.stringify(await this.serialize())
      }).then(r => r.json())
    } finally {
      return this
    }
  ...
  async download() {
    const json = await fetch(`https://project-bucket-hackclub.s3.eu-west-1.amazonaws.com/${this.id()}.json`, { mode: 'cors' }).then(r => r.json())
