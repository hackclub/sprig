...
static int read_command()
{
  ...
  for (;;)
  {
    ...
    {
      if (strlen(serial_commands[i]) > serial_command_indexes[i] && serial_commands[i][serial_command_indexes[i]] == c)
      {
...
void render_game_menu_screen(char *buffer, Welcome_State welcome_state)
{
  ...
  game_padding[SCREEN_WIDTH_CHARS - count_digits(welcome_state.games_i + 1) - count_digits(welcome_state.games_len) - 8 // 7 chars used for " Game: " + account for slash
  ] = '\0';

  size_padding[SCREEN_WIDTH_CHARS - count_digits(GAME_SLOTS(welcome_state.games[welcome_state.games_i].size_b)) - count_digits(MAX_SLOTS) - 8 // 7 chars used for " Size: " + account for slash
  ] = '\0';
...
void update_welcome_state(Welcome_State *welcome_state)
{
  ...
  if (welcome_state->screen == GAME_MENU)
    ...
    {
    case Button_A:
      if (welcome_state->games_i > 0)
        welcome_state->games_i--;
      break;
    case Button_D:
      if (welcome_state->games_i < welcome_state->games_len - 1)
        welcome_state->games_i++;
      break;
    case Button_S:
