...
case $1 in
    "--log") docker build ./docker | tee dockerBuildLog.txt
             docker run --security-opt label:disable --rm --volume `pwd`:/root/spade $(docker images | awk '{print $3}' | awk 'NR==2') | tee -a dockerBuildLog.txt ;;
    *) docker build ./docker
       docker run --security-opt label:disable --rm --volume `pwd`:/root/spade $(docker images | awk '{print $3}' | awk 'NR==2') ;;
esac
