...
function displayTime() {
  clearText();
  addText(`${timeRemaining}`, { x: 1, y: 1, color: color`2` });
}
...
function endGame() {
  ...
  addText(`You Lost. Score: ${level}`, { x: Math.floor(width() / 2) - 3, y: Math.floor(height() / 2), color: color`0` });
}
