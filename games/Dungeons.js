...
You can get softlocked, but that would be your fault.
...
function levelUp(px, py) {
  ...
  flashTimer = false
  timer += Math.round(
    hardness * (1+Math.round(Math.random()*5)) * Math.max(3, (10-level)*2) * 0.1
  )
  makeMap(px, py)
}
function init() {
