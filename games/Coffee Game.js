...
addText("Press S to start\n      or restart", {x: 2, y: 11, color: color`3` });
...
onInput("s", () => {
  ...
  const timer = setInterval(() => {
    ...
    addText("Order:" + item, {x: 1, y: 3, color: color`3`});
    addText("  w\nempty", {x: 3, y: 4, color: color`D`});
    addText("  d\nsend", {x: 3, y: 7, color: color`6`});
    addText(" i\nice", {x: 13, y: 6, color: color`5`});
    addText("  l\nwater", {x: 15, y: 8, color: color`7`});
    addText("    k\nespresso", {x: 10, y: 9, color: color`0`});
    addText(" j\nmilk", {x: 10, y: 8, color: color`2`});
