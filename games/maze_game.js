...
setSolids([player, wall, moveable_wall, door, door2, door3]);
...
addText("?? keys are portals", {x: 1, y: 11, color: color`3`});
...
afterInput(() => {
  ...
  if (level < levels.length-1) {
    ...
    if (tilesWith(restart, player).length > 1) {
      // console.log("setting map to 0 (restart)");
      level = 0;
      keys_needed = objectives[level];
      keys_collected = 0;
  ...
  if (level < levels.length-1) {
    ...
    addText(`collected ${gems_collected}/${levels.length-1} gems.`, {x: 1, y: 8, color: color`3`});
  } 
});