...
Climb the layers of the dungeon before the time runs out, collecting coins and avoiding traps!
See how far you can go and brag to your friends about it!
  ...
  const scr = Array.from({ length: MAZESZE*2-1 }, (_, row) =>
    Array.from({ length: MAZESZE*2-1 }, (_, col)=> {
      if ((col+row)%2 == 0) {
        ...
        const thisP = [col/2, row/2]
        if (thisP[0] == pos[0] && thisP[1] == pos[1]) {
          return player
        }
        if (thisP[0] == end[0] && thisP[1] == end[1]) {
          return stairs
        }
        if (getWall(...thisP, -1, 0) && getWall(...thisP, 1, 0) && 
            getWall(...thisP, 0, -1) && getWall(...thisP, 0, 1)) {
          return block
        }
        if (visitedLvls.some((p)=> p[0] === thisP[0] && p[1] === thisP[1])) {
          return brickCr
        }
        return brickD
      }
...
function makeMap(playerx, playery) {
  ...
  var scr = Array.from({ length: MAPSZE }, (_, row) =>
  ...
  if (visitedLvls.some((p)=> p[0] === pos[0] && p[1] === pos[1])) {
    scr = scr.map(it=>it.map(it=>{
      if (it == coin) {
        return brickD
      }
      return it
