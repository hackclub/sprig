...
function characterCollection() {
  ...
  else if (tilesWith(player, jared.char).length == 1) {
    collectCharacter(jared, collectedCharacters, 1);
  }
  else if (tilesWith(player, gilfoyle.char).length == 1) {
    collectCharacter(gilfoyle, collectedCharacters, 2);
  }
  else if (tilesWith(player, dinesh.char).length == 1) {
    collectCharacter(dinesh, collectedCharacters, 3);
  }
  else if (tilesWith(player, monica.char).length == 1) {
...
const originalFunction = characterCollection; // save a reference to the original function
...
function fall(char) {
  ...
  if (block_under_player.length < 1) {
    setTimeout(() => {getFirst(char).y += 1;}, 150)
...
ff.....................fff
//////qqqq////////////////
ffffffffffffffffffffffffff`, // lvl 5
  map`
abcde.............
..................
......qqq.qq......
...
let set = [player, erlichBag, jaredBag, gilfoyleBag, dineshBag, monicaBag, white, black, neonGreen]
setSolids(set)
...
afterInput(() => {
  if (level == 6) {
    ...
    if (tilesWith(player,goal).length == 1) {
      setTimeout(() => {level = level + 1; setMap(levels[level]);}, 100)
    }
    
    richardDied();
    displayCollectedCharacters();

    characterCollection = originalFunction;
    characterCollection();
  }
