...
const hocksound = tune`
159.5744680851064: C5~159.5744680851064,
159.5744680851064: D5~159.5744680851064,
4787.234042553192`;
...
function maingame() {
  ...
  getAll(border).forEach((obstacle) => {
    if (obstacle.x === 0) {
      obstacle.remove();
    } else {
      obstacle.x -= 1;
  ...
  if (getAll(border).length === 0) {
    generateObstacle();
  }

  hocky().type = [FLAP_UP, FLAP_DOWN][count % 2];

  if (getAll(border).some((obstacle) => obstacle.x === hocky().x && obstacle.y === hocky().y)) {
    gameOver();
  }
