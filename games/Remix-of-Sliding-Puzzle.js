/*
@title: Sliding_Puzzle_REMIX
@author: Ruben Galvan
@tags: [puzzle, forest]
@addedOn: 2026-08-8
@description: A remix built upon Eren Karababa's Classic Sliding puzzle game! Features refined sprites, dark forest color themes, and a noise effect win animation.
*/

const player = "p";
const tile1 = "1";
const tile2 = "2";
const tile3 = "3";
const tile4 = "4";
const tile5 = "5";
const tile6 = "6";
const tile7 = "7";
const tile8 = "8";
const empty = "e";
const bg = "b";

setLegend(
  [player, bitmap`
................
................
................
................
................
................
................
................
................
................
................
................
................
................
................
................`],
  // Forest palette:
  // 0: Black / Outer Border
  // 4: Dark Forest Green Background
  // L: Numbers
  [tile1, bitmap`
0000000000000000
0444444444444440
0444444444444440
0444444LL4444440
044444LLLL444440
04444443LL444440
04444444LL444440
04444444LL444440
04444444LL444440
04444444LL444440
044444LLLLLL4440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0000000000000000`],
  [tile2, bitmap`
0000000000000000
0444444444444440
0444444444444440
04444LLLLLL44440
0444LL4444LL4440
0444444444LL4440
044444444LL44440
044444LLLL444440
04444LL444444440
0444LL4444444440
0444LLLLLLLL4440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0000000000000000`],
  [tile3, bitmap`
0000000000000000
0444444444444440
0444444444444440
04444LLLLLL44440
0444LL4444LL4440
0444444444LL4440
044444LLLLLL4440
0444444444LL4440
0444LL4444LL4440
04444LLLLLL44440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0000000000000000`],
  [tile4, bitmap`
0000000000000000
0444444444444440
0444444444444440
044444LL4LL44440
04444LL44LL44440
0444LL444LL44440
0444LLLLLLLL4440
044444444LL44440
044444444LL44440
044444444LL44440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0000000000000000`],
  [tile5, bitmap`
0000000000000000
0444444444444440
0444444444444440
0444LLLLLLLL4440
0444LL4444444440
0444LLLLLL444440
0444444444LL4440
0444444444LL4440
0444LL4444LL4440
04444LLLLLL44440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0000000000000000`],
  [tile6, bitmap`
0000000000000000
0444444444444440
0444444444444440
044444LLLLLL4440
04444LL444444440
0444LL4444444440
0444LLLLLLLL4440
0444LL4444LL4440
0444LL4444LL4440
04444LLLLLL44440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0000000000000000`],
  [tile7, bitmap`
0000000000000000
0444444444444440
0444444444444440
0444LLLLLLLL4440
0444444444LL4440
044444444LL44440
04444444LL444440
0444444LL4444440
044444LL44444440
044444LL44444440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0000000000000000`],
  [tile8, bitmap`
0000000000000000
0444444444444440
0444444444444440
04444LLLLLL44440
0444LL4444LL4440
0444LL4444LL4440
04444LLLLLL44440
0444LL4444LL4440
0444LL4444LL4440
04444LLLLLL44440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0444444444444440
0000000000000000`],
  [empty, bitmap`
0000000000000000
0000000000000000
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0044444444444400
0000000000000000
0000000000000000`],
  // Solid Dark Forest Green Background
  [bg, bitmap`
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444
4444444444444444`]
);

setSolids([tile1, tile2, tile3, tile4, tile5, tile6, tile7, tile8]);

let level = 0;
const levels = [
  map`
bbb
bbb
bbb`,
];

setMap(levels[level]);
setBackground(bg);

let gameStarted = false;
let gameWon = false;
let noiseInterval = null;

function showMenu() {
  clearText();
  addText("Forest Puzzle", { y: 2, color: color`4` });
  addText("Arrange tiles", { y: 5, color: color`1` });
  addText("in order 1-8", { y: 6, color: color`1` });
  addText("Use WASD keys", { y: 9, color: color`1` });
  addText("to slide tiles", { y: 10, color: color`1` });
  addText("into the empty", { y: 11, color: color`1` });
  addText("space", { y: 12, color: color`1` });
  addText("Press W to", { y: 14, color: color`4` });
  addText("START!", { y: 15, color: color`4` });
}

function stopNoiseEffect() {
  if (noiseInterval) {
    clearInterval(noiseInterval);
    noiseInterval = null;
  }
}

function startNoiseEffect() {
  stopNoiseEffect();

  const sprites = [tile1, tile2, tile3, tile4, tile5, tile6, tile7, tile8, empty];

  noiseInterval = setInterval(() => {
    for (let x = 0; x < 3; x++) {
      for (let y = 0; y < 3; y++) {
        clearTile(x, y);
        const randomSprite = sprites[Math.floor(Math.random() * sprites.length)];
        addSprite(x, y, randomSprite);
      }
    }
  }, 100);
}

function initPuzzle() {
  stopNoiseEffect();
  gameWon = false;
  clearText();

  clearTile(0, 0); addSprite(0, 0, tile1);
  clearTile(1, 0); addSprite(1, 0, tile2);
  clearTile(2, 0); addSprite(2, 0, tile3);
  clearTile(0, 1); addSprite(0, 1, tile4);
  clearTile(1, 1); addSprite(1, 1, tile5);
  clearTile(2, 1); addSprite(2, 1, tile6);
  clearTile(0, 2); addSprite(0, 2, tile7);
  clearTile(1, 2); addSprite(1, 2, tile8);
  clearTile(2, 2); addSprite(2, 2, empty);

  for (let i = 0; i < 50; i++) {
    const emptyTile = getFirst(empty);
    const moves = [];

    if (emptyTile.x > 0) moves.push([-1, 0]);
    if (emptyTile.x < 2) moves.push([1, 0]);
    if (emptyTile.y > 0) moves.push([0, -1]);
    if (emptyTile.y < 2) moves.push([0, 1]);

    const move = moves[Math.floor(Math.random() * moves.length)];
    swapTiles(emptyTile.x + move[0], emptyTile.y + move[1]);
  }
}

function swapTiles(x, y) {
  const emptyTile = getFirst(empty);
  const tiles = getTile(x, y);

  if (tiles.length > 0) {
    const tile = tiles[0];
    clearTile(emptyTile.x, emptyTile.y);
    clearTile(x, y);
    addSprite(emptyTile.x, emptyTile.y, tile.type);
    addSprite(x, y, empty);
  }
}

function checkWin() {
  const positions = [
    {x: 0, y: 0, t: tile1}, {x: 1, y: 0, t: tile2}, {x: 2, y: 0, t: tile3},
    {x: 0, y: 1, t: tile4}, {x: 1, y: 1, t: tile5}, {x: 2, y: 1, t: tile6},
    {x: 0, y: 2, t: tile7}, {x: 1, y: 2, t: tile8}, {x: 2, y: 2, t: empty}
  ];

  for (const pos of positions) {
    const tiles = getTile(pos.x, pos.y);
    if (tiles.length === 0 || tiles[0].type !== pos.t) {
      return false;
    }
  }
  return true;
}

function showWinScreen() {
  gameWon = true;
  startNoiseEffect();

  addText("You Win!", { y: 4, color: color`4` });
  addText("Press J to", { y: 6, color: color`1` });
  addText("play again!", { y: 7, color: color`1` });
}

function tryMove(dx, dy) {
  if (!gameStarted || gameWon) return;

  const emptyTile = getFirst(empty);
  const targetX = emptyTile.x + dx;
  const targetY = emptyTile.y + dy;

  if (targetX >= 0 && targetX < 3 && targetY >= 0 && targetY < 3) {
    swapTiles(targetX, targetY);
    if (checkWin()) {
      showWinScreen();
    }
  }
}

function startGame() {
  gameStarted = true;
  initPuzzle();
}

showMenu();

onInput("w", () => {
  if (!gameStarted) {
    startGame();
  } else {
    tryMove(0, 1);
  }
});

onInput("s", () => tryMove(0, -1));
onInput("a", () => tryMove(1, 0));
onInput("d", () => tryMove(-1, 0));

onInput("j", () => {
  if (gameStarted) {
    initPuzzle();
  }
});
