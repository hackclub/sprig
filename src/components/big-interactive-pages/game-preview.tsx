...
const shouldShowConflict = (
	...
	const lastSavedData = lastSavedSessionInfo
		? JSON.parse(lastSavedSessionInfo)
		: null;
...
const performSave = async (
	...
	try {
		...
		localStorage.setItem(
			LAST_SAVED_SESSION_ID,
			JSON.stringify({ sessionId: sessionId, gameId: game.id })
		);
...
export async function startSavingGame(
	persistenceState: Signal<PersistenceState>,
...
const exitTutorial = (
	...
	if (persistenceState.value.kind === "PERSISTED") {
		...
		if (isNewSaveStrat.value) startSavingGame(persistenceState, undefined);
		else
			saveGame(
				persistenceState,
...
export default function GamePreview({
	...
	useEffect(() => {
		...
		channel.onmessage = (event) => {
			if (
				event.data.type === "SESSION_UPDATE" &&
	...
	useEffect(() => {
		if (screen.current) {
			...
			loadThumbnailUrl(urlArray[urlArray.length - 1] || "").then(
				(res) => {
					const context = screen.current.getContext("2d");
	...
	if (
		...
		persistenceState.value.kind === "COLLAB" &&
		typeof persistenceState.value.game === "string"
	)
		return <RoomPasswordPopup persistenceState={persistenceState} />;
	else
		return (
						...
						>
							...
							>
								...
								<div class={styles.metaData}>
									<h1>{persistenceState.value.name}</h1>
									<p style={{ fontStyle: "italic" }}>
										by: {persistenceState.value.authorName}
									</p>
								</div>
							</div>
						</div>
