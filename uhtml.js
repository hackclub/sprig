...
const diffable = (node1, operation)=>node1.nodeType === 111 ? 1 / operation < 0 ? operation ? remove(node1) : node1.lastChild : operation ? node1.valueOf() : node1.firstChild : node1
;
...
const __default2 = (parentNode, a, b, get, before)=>{
    ...
    while(aStart < aEnd || bStart < bEnd){
        if (aEnd === aStart) {
            const node1 = bEnd < bLength ? bStart ? get(b[bStart - 1], -0).nextSibling : get(b[bEnd - bStart], 0) : before;
            ...
            if (map.has(a[aStart])) {
                ...
                if (bStart < index && index < bEnd) {
                    ...
                    if (sequence > index - bStart) {
                        ...
                        while(bStart < index)parentNode.insertBefore(get(b[bStart++], 1), node1);
                    } else {
    ...
    return (newValue)=>{
        if (oldValue !== newValue) {
            ...
            if (oldValue == null) {
                ...
                const value1 = useForeign && newValue instanceof Foreign ? newValue._(node1, name) : newValue;
                if (value1 == null) {
...
const event = (node1, name)=>{
    ...
    if (!(name in node1) && (lower = name.toLowerCase()) in node1) type = lower.slice(2);
    return (newValue)=>{
        const info = isArray(newValue) ? newValue : [
            newValue,
            false
        ];
        if (oldValue !== info[0]) {
...
const setter = (node1, key)=>key === 'dataset' ? data(node1) : (value1)=>{
        node1[key] = value1;
    }
;
const text = (node1)=>{
...
;
const diff = (comment, oldNodes, newNodes)=>__default2(comment.parentNode, oldNodes, newNodes, diffable, comment)
;
const handleAnything = (comment)=>{
    let oldValue, text1, nodes = [];
        ...
        switch(typeof newValue){
            ...
            case 'undefined':
                ...
                if (oldValue !== newValue && 'ELEMENT_NODE' in newValue) {
                    oldValue = newValue;
                    nodes = diff(comment, nodes, newValue.nodeType === 11 ? slice.call(newValue.childNodes) : [
                        newValue
                    ]);
                }
                break;
...
function handlers(options) {
    ...
    return type === 'node' ? handleAnything(node1) : type === 'attr' ? handleAttribute(node1, options.name) : text(node1);
}
...
const createFragment = isImportNodeLengthWrong ? (text1, type, normalize)=>document.importNode(createContent(text1, type, normalize), true)
 : createContent;
const createWalker = isImportNodeLengthWrong ? (fragment)=>document.createTreeWalker(fragment, 1 | 128, null, false)
...
const mapTemplate = (type, template)=>{
    ...
    while(i < length){
        ...
        if (node1.nodeType === 8) {
            ...
            if (textOnly.test(node1.tagName) && node1.textContent.trim() === `<!--${search}-->`) {
                node1.textContent = '';
                nodes.push({
                    type: 'text',
                    path: createPath(node1)
...
const unrollValues = ({ stack  }, values, length)=>{
    for(let i = 0; i < length; i++){
        const hole = values[i];
        if (hole instanceof Hole1) values[i] = unroll(stack[i] || (stack[i] = createCache()), hole);
...
const tag = (type)=>{
    ...
    const fixed = (cache1)=>(template, ...values)=>unroll(cache1, {
                type,
                template,
                values
            })
    ;
    return defineProperties((template, ...values)=>new Hole1(type, template, values)
    , {
        for: {
            value (ref, id) {
        ...
        node: {
            value: (template, ...values)=>unroll(createCache(), {
                    type,
                    template,
                    values
                }).valueOf()
...
const svg1 = tag('svg');
export { Hole1 as Hole, render1 as render, html1 as html, svg1 as svg, foreign1 as foreign };