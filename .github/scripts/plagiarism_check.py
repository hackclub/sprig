...
def run_compare50(args):
    single_file, file, output_dir, saved_dir_base = args
    try:
...
def main():
    ...
    num_workers = min(cpu_count(), len(all_js_files))
    pool = Pool(num_workers)
    pool.map(run_compare50, [(single_file, file, output_dir, saved_dir_base) for file in all_js_files])
    pool.close()
    pool.join()
