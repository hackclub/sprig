...
function nextLevel() {
  ...
  if(lost){
    ...
    if (level < levels.length) {
      ...
      gameOver = false;
      clearText();
      addText("All Levels Complet!!", { y: 5, color: color`4` });
    }
  }
}
...
function moveCatTowardsPlayer() {
  ...
  if (playerTile.y < catTile.y && !getTile(catTile.x, catTile.y - 1).some(s => s.type === cornerwall)) {
    catTile.y -= 1;
